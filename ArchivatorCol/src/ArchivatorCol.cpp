...
myUnorderedMap <char,string> binaryTable;
...
void buildBinaryTree(){
    ...
    while(binaryTree.size()>1){
        binaryTree.sort(compareFunction);
        ...
        Node tmp=nodes[newNode.left];
        nodes[newNode.right].top=nodes.size()-1;
        tmp=nodes[newNode.right];
...
void buildElemBinaryTable(){
    for(int i=0; i<nodes.size(); i++){
        ...
        binaryTable.insert((char)nodes[i].elem,res);
    }
}
...
void writeArchivedFile(string &filename,auto &vbsData){
    ...
    fout << sourceExt << "|"; // save source extension & delimiter

    // save bin. table
    for(int i=0; i<binaryTable.size();i++){
        string s8bit=binaryTable.data(i);
        ...
        if(s8bit.size()>8){
            ...
            fout << sourceBits << binaryTable.key(i) << (unsigned char) bitElem.to_ulong() << (unsigned char) bitElem2.to_ulong();
        } else {
            bitset<8> bitElem (s8bit);
            fout << sourceBits << binaryTable.key(i) << (unsigned char) bitElem.to_ulong();
        }
    }
    fout << "|"; // delimiter
    // save data
    for(int i=0;i<vbsData.size();i++){
        auto elem=vbsData[i];
...
void pack(string filename){
    ...
    if(outBS.size()%8>0)
        arcSize++;
    myVector<bitset<8>> vbsData (arcSize);
...
string searchInBinaryTable(string sBits){
    string res="";
    for(int i=0;i<binaryTable.size();i++)
        if(binaryTable.data(i)==sBits){
            res=binaryTable.key(i);
            break;
        }
    return res;
}
...
void writeUnpackedFile(string filename, string sourceExt, auto &realSourceData){
    ...
    if(fileExists(filename))
        deleteFile(filename);
    ofstream fout(filename, ofstream::binary);
    for(int i=0; i<realSourceData.size();i++){
        auto el=realSourceData[i];
...
void readSourceBinaryTable(auto &buffer, int &i, int length){
    ...
    while(i<length){
        ...
        if(counter==0)
            ...
            binaryTable.insert(curChar,sBits);
        }
