...
myVector<Pnt> getNewObject(int startX, int startY){
    myVector<Pnt> res;
    ...
    myQueue <Pnt> qWorkingPoints;
    qWorkingPoints.add(currentPoint);
    image->setRGB(currentPoint.x, currentPoint.y, 0xffffff);// repaint point to white
    ...
    for(int y=0; y<imgHeight-1;y++)
        for(int x=0; x<imgWidth-1;x++){
            if(isSilhouetteColor(image->getRGB(x, y))){
                myVector<Pnt> object = getNewObject(x,y);
