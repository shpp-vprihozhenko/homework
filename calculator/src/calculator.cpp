...
// expression example: 2.2+5*(6-2)*cos(30+30)*2^3+14
// for graphic enter: y=x*sin(x)
...
float makeOperation(float n1, float n2, char oper){
    float res=0;
    if (oper=='+')
        res=n1+n2;
    else if (oper=='-')
...
float countExpression(string exp){
    stack <float> stOperands;
    stack <char> stOperators;
    string obj="";
    char lastOperator=0;
...
string enterExpression(){
    string res="";
    cout << "Enter expression to calculate (or y=... for graphic): ";
    cin >> res;
    return res;
}
